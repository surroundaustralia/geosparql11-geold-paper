...
The Open Geospatial Consortium (OGC) \& World Wide Web Consortium's (W3C) \textit{Spatial Data On The Web Working Group}
(SDWWG) established \textit{Spatial Data On The Web Best Practices}\cite{van_den_brink_best_2018} which noted 
shortcomings with then current spatial data standards such as ``A best practice for returning geometries in a 
...
to GeoSPARQL were ultimately made by that Working Group.
...
of a spatial object that can be used for different purposes'', again, this is being considered by the SWG with initial
thoughts cenetering on defining \textit{roles} of geometries with respect to features.
...
Figure~\ref{fig:geosparql11ontology} overviews the ontology, as 
of release 1.1.
...
The 1.1 release also adds the property \href{http://www.opengis.net/def/function/geosparql/inSRS}{\texttt{geo:inSRS}}, which allows declarations of a geometry's SRS, independent of serializations. It paves the way 
...
An staged schedule of updates to this very important \textit{Semantic Web} spatial data standard has been initiated with simple, objvious and strictly backwards-compatable changes now in progress for GeoSPARQL 1.1. 1.2 will include more detailed work later in 2021 and 2.0, as yet un-specified, might be a somewhat different stanandard, to be determined by more requirements gathering and feedback on the 1.1 and 1.2 releases.
